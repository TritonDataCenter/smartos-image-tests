...
# Test to ensure files and directories under /etc are not owned by the
...
# by the 1000 user.
describe command('find /etc -user 1000 -ls | grep 1000') do
  its(:exit_status) { should eq 1 }
end


# Test to ensure files directories under /var are not owned by the
# '1000' user (the owner should be root)
# See IMAGE-743 and IMAGE-744
